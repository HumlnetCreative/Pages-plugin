                ...
                <div class="hucr-slider-media__uploads">
                    <?php foreach (['mp4' => ['MP4 video (povinné)', $mp4], 'webm' => ['WebM video (volitelné)', $webm]] as $format => [$label, $use]): ?>
                        <div class="hucr-slider-media__upload" id="slider-video-<?= $context->id ?>-<?= $format ?>">
                            <label><?= e($label) ?></label><input type="file" class="form-control" name="slider_media_file" accept="video/<?= $format ?>">
                            <input type="hidden" name="context_id" value="<?= $context->id ?>"><input type="hidden" name="format" value="<?= $format ?>">
                            <button type="button" class="btn btn-primary" data-request="<?= e($handler('onUploadVideo')) ?>" data-request-files data-request-form="#slider-video-<?= $context->id ?>-<?= $format ?>">Nahrát <?= strtoupper($format) ?></button>
                ...
                <div class="hucr-slider-media__uploads">
                    <?php foreach (['desktop' => ['Poster pro počítač', $posterDesktop], 'mobile' => ['Poster pro mobil', $posterMobile]] as $viewport => [$label, $use]): ?>
                        <div class="hucr-slider-media__upload" id="slider-poster-<?= $context->id ?>-<?= $viewport ?>">
                            <label><?= e($label) ?></label><input type="file" class="form-control" name="slider_media_file" accept="image/jpeg,image/png,image/webp,image/avif">
                            <input type="hidden" name="context_id" value="<?= $context->id ?>"><input type="hidden" name="viewport" value="<?= $viewport ?>">
                            <button type="button" class="btn btn-default" data-request="<?= e($handler('onUploadPoster')) ?>" data-request-files data-request-form="#slider-poster-<?= $context->id ?>-<?= $viewport ?>">Nahrát poster</button>
                ...
                <div class="hucr-slider-media__uploads">
                    <?php foreach (['desktop' => ['Obrázek pro počítač', $desktop], 'mobile' => ['Obrázek pro mobil', $mobile]] as $viewport => [$label, $use]): ?>
                        <div class="hucr-slider-media__upload" id="slider-image-<?= $context->id ?>-<?= $viewport ?>">
                            <label><?= e($label) ?></label><input type="file" class="form-control" name="slider_media_file" accept="image/jpeg,image/png,image/webp,image/avif">
                            <input type="hidden" name="context_id" value="<?= $context->id ?>"><input type="hidden" name="viewport" value="<?= $viewport ?>">
                            <button type="button" class="btn btn-primary" data-request="<?= e($handler('onUploadImage')) ?>" data-request-files data-request-form="#slider-image-<?= $context->id ?>-<?= $viewport ?>">Nahrát obrázek</button>
                            ...
                            <?php if ($use): ?>
                                ...
                                <button type="button" class="btn btn-primary btn-sm" data-request="<?= e($handler('onUpdateImageMetadata')) ?>" data-request-data="context_id: <?= $context->id ?>, media_use_id: <?= $use->id ?>" data-request-form="#slider-image-<?= $context->id ?>-<?= $viewport ?>">Uložit popis</button>
